...
function get_bytes(inTxt)
{
	...
	for (var i = 0; i < inSplit.length; i++)
	{
		...
		{
			try {
				var x = this.parseInt(inSplit[i], 16);
				if (x >= 0 && x <= 0xFF) {
					inVals.push(x);
				}
			}
...
class Parser {
  constructor() {
    this.indent = 0;
   	this.usagePage = Array();
   	this.collection = Array();
   	this.stackPtr = 0;
    this.possible_errors = 0;
  }

  go_parse_hidrepdesc(inTxt)
  {
	// TODO try
	{
		this.possible_errors = 0;
		...
		for (var i = 0; i < inVals.length; )
		{
			...
			if (bType == 0x03 && bTag == 0x0F && bSize == 2 && i + 2 < inVals.length)
			{
				...
				outTxt += this.pHexC(b0) + this.pHexC(bDataSize) + this.pHexC(bLongItemTag) + this.pindentComment("Long Item (" + this.pHex(bLongItemTag) + ")", 2);
				for (var j = 0; j < bDataSize && i < inVals.length; j++) {
					outTxt += this.pHexC(inVals[i++]);
					this.possible_errors++; // there are no devices that use long item data right now
				}
				this.indent++;
				outTxt += this.pindentComment("Long Item Data (" + bDataSize.toString(10) + " bytes)", 2);
				this.indent--;
			...
			else
			{
				var bSizeActual = 0;
				var itemVal = 0;
				outTxt += this.pHexC(b0);
				for (var j = 0; j < bSize; j++)
				{
					if (i + j < inVals.length) {
						outTxt += this.pHexC(inVals[i + j]);
				...
				if (bType == 0x00)
				{
					if (bTag == 0x08)
					{
						outTxt += this.pindentComment("Input" + this.pInputOutputFeature(bSize, itemVal, bTag), bSizeActual);
					}
					else if (bTag == 0x09)
					{
						outTxt += this.pindentComment("Output" + this.pInputOutputFeature(bSize, itemVal, bTag), bSizeActual);
					}
					else if (bTag == 0x0B)
					{
						outTxt += this.pindentComment("Feature" + this.pInputOutputFeature(bSize, itemVal, bTag), bSizeActual);
					}
					else if (bTag == 0x0A)
					{
						this.collection[this.stackPtr] = itemVal;
						outTxt += this.pindentComment("collection" + this.pcollection(bSize, itemVal), bSizeActual);
						this.indent++;
					}
					else if (bTag == 0x0C)
					{
						this.indent--;
						outTxt += this.pindentComment("End collection", bSizeActual);
					}
					else
					{
						outTxt += this.pindentComment("Unknown (bTag: " + this.pHex(bTag) + ", bType: " + this.pHex(bType) + ")", bSizeActual);
				...
				else if (bType == 0x01)
				{
					if (bTag == 0x00)
					{
						this.usagePage[this.stackPtr] = itemVal;
						outTxt += this.pindentComment("Usage Page" + this.pusagePage(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x01)
					{
						outTxt += this.pindentComment("Logical Minimum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x02)
					{
						outTxt += this.pindentComment("Logical Maximum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x03)
					{
						outTxt += this.pindentComment("Physical Minimum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x04)
					{
						outTxt += this.pindentComment("Physical Maximum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x05)
					{
						outTxt += this.pindentComment("Unit Exponent" + this.pUnitExp(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x06)
					{
						outTxt += this.pindentComment("Unit" + this.pUnit(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x07)
					{
						outTxt += this.pindentComment("Report Size" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x08)
					{
						outTxt += this.pindentComment("Report ID" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x09)
					{
						outTxt += this.pindentComment("Report Count" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x0A)
					{
						outTxt += this.pindentComment("Push", bSizeActual);
						this.indent++;
						this.stackPtr++;
						this.usagePage[this.stackPtr] = this.usagePage[this.stackPtr - 1];
						this.collection[this.stackPtr] = this.collection[this.stackPtr - 1];
					}
					else if (bTag == 0x0B)
					{
						this.indent--;
						this.stackPtr--;
						outTxt += this.pindentComment("Pop", bSizeActual);
					}
					else
					{
						outTxt += this.pindentComment("Unknown (bTag: " + this.pHex(bTag) + ", bType: " + this.pHex(bType) + ")", bSizeActual);
				...
				else if (bType == 0x02)
				{
					if (bTag == 0x00)
					{
						outTxt += this.pindentComment("Usage" + this.pUsage(bSize, itemVal, this.usagePage[this.stackPtr]), bSizeActual);
					}
					else if (bTag == 0x01)
					{
						outTxt += this.pindentComment("Usage Minimum" + this.pUsage(bSize, itemVal, this.usagePage[this.stackPtr]), bSizeActual);
					}
					else if (bTag == 0x02)
					{
						outTxt += this.pindentComment("Usage Maximum" + this.pUsage(bSize, itemVal, this.usagePage[this.stackPtr]), bSizeActual);
					}
					else if (bTag == 0x03)
					{
						outTxt += this.pindentComment("Designator Index" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x04)
					{
						outTxt += this.pindentComment("Designator Minimum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x05)
					{
						outTxt += this.pindentComment("Designator Maximum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x07)
					{
						outTxt += this.pindentComment("String Index" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x08)
					{
						outTxt += this.pindentComment("String Minimum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x09)
					{
						outTxt += this.pindentComment("String Maximum" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else if (bTag == 0x0A)
					{
						outTxt += this.pindentComment("Delimiter" + this.pItemVal(bSize, itemVal), bSizeActual);
					}
					else
					{
						outTxt += this.pindentComment("Unknown (bTag: " + this.pHex(bTag) + ", bType: " + this.pHex(bType) + ")", bSizeActual);
	...
	// TODO 	this.possible_errors++;
  // TODO   throw e;
  // TODO }
    return outTxt;
  }

  go_parse_stddesc(inTxt)
  {
	// TODO try
	{
		this.possible_errors = 0;
		...
		for (var i = 0; i < inVals.length; )
		{
			if (j <= 0)
			{
				bLength = inVals[i++];
				j = bLength - 1;
				outTxt += this.pHexC(bLength) + this.pindentComment("bLength", 1);
				if (bLength > inVals.length) {
					this.possible_errors += 2;
				}
			}
			else if (j == bLength - 1)
			{
				...
				{
					outTxt += this.pHexC(bDescriptorType) + this.pindentComment("bDescriptorType (See Next Line)", 1);
				}
				else
				{
					outTxt += this.pHexC(bDescriptorType) + this.pindentComment("bDescriptorType" + this.pDescriptorType(bDescriptorType), 1);
				}
			}
			else
			{
				if (bDescriptorType == 0x21)
				{
					...
					if (i < inVals.length && j > 0) {
						...
						bcdHID += bcdL + (bcdH * 10);
						i++; j--;
						outTxt += this.pindentComment("bcdHID " + parseFloat(Math.round(bcdHID * 100) / 100).toFixed(2), 2);
					}
					if (i < inVals.length && j > 0) {
						outTxt += this.pHexC(inVals[i]);
						outTxt += this.pindentComment("bCountryCode", 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bNumDescriptors", 1);
						i++; j--;
					}
					...
					for (var k = 0; k < bNumDescriptors; k++)
					{
						if (i < inVals.length && j > 0) {
							...
							var str = " (HID)";
							if (inVals[i] != 0x22) {
								this.possible_errors++; // for HID, this field must also be HID
								str = " (Unknown " + this.pHex(inVals[i]) + ")";
							}
							outTxt += this.pindentComment("bDescriptorType[" + k + "]" + str, 1);
							i++; j--;
						}
						...
						if (i < inVals.length && j > 0) {
							...
							outTxt += this.pindentComment("wDescriptorLength[" + k + "] " + wDescriptorLength.toString(10), 2);
							i++; j--;
						}
					}
				...
				else if (bDescriptorType == 0x29)
				{
					...
					if (i < inVals.length && j > 0) {
						outTxt += this.pHexC(inVals[i]);
						bNbrPorts = inVals[i];
						outTxt += this.pindentComment("bNbrPorts", 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("wHubCharacteristics", 2); // TODO
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bPwrOn2PwrGood " + (bPwrOn2PwrGood * 2) + "ms", 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bHubContrCurrent " + (bHubContrCurrent * 1) + "mA", 1);
						i++; j--;
					}
					...
					if (bNbrPorts > 0 && hasP) {
						outTxt += this.pindentComment("DeviceRemovable", Math.floor(bNbrPorts / 8) + 1);
					}
					...
					if (bNbrPorts > 0 && hasP) {
						outTxt += this.pindentComment("PortPwrCtrlMask", Math.floor(bNbrPorts / 8) + 1);
					}
				}
				else if (bDescriptorType == 0x01 || bDescriptorType == 0x06)
				{
					...
					if (i < inVals.length && j > 0) {
						...
						bcdUSB += bcdL + (bcdH * 10);
						i++; j--;
						outTxt += this.pindentComment("bcdUSB " + parseFloat(Math.round(bcdUSB * 100) / 100).toFixed(2), 2);
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bDeviceClass " + this.pDeviceClass(bDeviceClass), 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bDeviceSubClass " + this.pDeviceSubClass(bDeviceClass, bDeviceSubClass), 1); // TODO
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bDeviceProtocol " + this.pDeviceProtocol(bDeviceClass, bDeviceSubClass, bDeviceProtocol), 1); // TODO
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bMaxPacketSize0 " + bMaxPacketSize0.toString(10), 1);
						i++; j--;
					}
					...
					if (bDescriptorType == 0x01)
					{
						...
						if (i < inVals.length && j > 0) {
							...
							idVendor += inVals[i] << 8;
							i++; j--;
							outTxt += this.pindentComment("idVendor " + this.pHex(idVendor), 2);
						}
						...
						if (i < inVals.length && j > 0) {
							...
							i++; j--;
							outTxt += this.pindentComment("idProduct " + this.pHex(idProduct), 2);
						}
						...
						if (i < inVals.length && j > 0) {
							...
							i++; j--;
							outTxt += this.pindentComment("bcdDevice " + parseFloat(Math.round(bcdDevice * 100) / 100).toFixed(2), 2);
						}
						...
						if (i < inVals.length && j > 0) {
							...
							outTxt += this.pindentComment("iManufacturer (String Index)", 1);
							i++; j--;
						}
						...
						if (i < inVals.length && j > 0) {
							outTxt += this.pHexC(inVals[i]);
							iProduct = inVals[i];
							outTxt += this.pindentComment("iProduct (String Index)", 1);
							i++; j--;
						}
						...
						if (i < inVals.length && j > 0) {
							...
							outTxt += this.pindentComment("iSerialNumber (String Index)", 1);
							i++; j--;
						}
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bNumConfigurations " + bNumConfigurations.toString(10), 1);
						i++; j--;
					}
					...
					if (bDescriptorType == 0x06)
					{
						if (i < inVals.length && j > 0) {
							outTxt += this.pHexC(inVals[i]);
							outTxt += this.pindentComment("bReserved", 1);
							i++; j--;
						}
					}
				}
				else if (bDescriptorType == 0x02 || bDescriptorType == 0x07) // config or other speed config
				{
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bNumInterfaces " + bNumInterfaces.toString(10), 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bConfigurationValue", 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("iConfiguration (String Index)", 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bmAttributes" + str, 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						outTxt += this.pHexC(inVals[i]);
						bMaxPower = inVals[i];
						outTxt += this.pindentComment("bMaxPower " + (bMaxPower * 2) + "mA", 1);
						i++; j--;
					}
				}
				else if (bDescriptorType == 0x04) // interface
				{
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bInterfaceNumber " + bInterfaceNumber.toString(10), 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bAlternateSetting", 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bNumEndpoints " + bNumEndpoints.toString(10), 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bInterfaceClass" + this.pInterfaceClass(bInterfaceClass), 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bInterfaceSubClass" + this.pInterfaceSubClass(bInterfaceClass, bInterfaceSubClass), 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bInterfaceProtocol", 1); // TODO
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						outTxt += this.pHexC(inVals[i]);
						iInterface = inVals[i];
						outTxt += this.pindentComment("iInterface (String Index)", 1);
						i++; j--;
					}
				}
				else if (bDescriptorType == 0x05) // endpoint
				{
					...
					if (i < inVals.length && j > 0)
					{
						...
						outTxt += this.pindentComment("bEndpointAddress" + str, 1);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("bmAttributes" + str, 1); // TODO
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						...
						outTxt += this.pindentComment("wMaxPacketSize " + wMaxPacketSize_.toString(10) + str, 2);
						i++; j--;
					}
					...
					if (i < inVals.length && j > 0) {
						outTxt += this.pHexC(inVals[i]);
						bInterval = inVals[i];
						outTxt += this.pindentComment("bInterval " + bInterval.toString(10) + " (unit depends on device speed)", 1); // TODO
						i++; j--;
					}
					...
					if (bInterfaceClass == 0x01 && bInterfaceSubClass == 0x02)
					{
						...
						if (i < inVals.length && j > 0) {
							outTxt += this.pHexC(inVals[i]);
							bRefresh = inVals[i];
							outTxt += this.pindentComment("bRefresh", 1);
							i++; j--;
						}
						var bSyncAddress = -1;
						if (i < inVals.length && j > 0) {
							...
							outTxt += this.pindentComment("bSyncAddress", 1);
							i++; j--;
						}
					}
				}
				else if (bDescriptorType == 0x25 && bInterfaceClass == 0x01 && bInterfaceSubClass == 0x02)
				{
					...
					if (i < inVals.length && j > 0)
					{
						...
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_ENDPOINT -> EP_GENERAL)", 1);
							i++; j--;
							var bmAttributes = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								...
								outTxt += this.pindentComment("bmAttributes" + atrstr, 1);
								i++; j--;
							}
							...
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("bLockDelayUnits", 1);
								i++; j--;
							}
							var wLockDelay = -1;
							if (i < inVals.length && j > 0) {
							...
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("wLockDelay " + wLockDelay.toString(10), 2);
								i++; j--;
							}
						}
						else
						{
							outTxt += this.pindentComment("bDescriptorSubtype Unknown", 1);
							i++; j--;
						}
					}
				}
				else if (bDescriptorType == 0x24 && bInterfaceClass == 0x01 && bInterfaceSubClass == 0x01)
				{
					...
					if (i < inVals.length && j > 0)
					{
						...
						if (bDescriptorSubtype == 0x01)
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_INTERFACE -> HEADER)", 1);
							i++; j--;
							var bcdADC = 0;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
							...
							if (i < inVals.length && j > 0) {
								...
								bcdADC += bcdL + (bcdH * 10);
								i++; j--;
								outTxt += this.pindentComment("bcdADC " + parseFloat(Math.round(bcdADC * 100) / 100).toFixed(2), 2);
							}
							var wTotalLength = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
							...
							while (j > 0)
							{
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("baInterfaceNr " + baInterfaceNr.toString(10), 1);
									i++; j--;
								}
							}
						}
						...
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_INTERFACE -> INPUT_TERMINAL)", 1);
							i++; j--;
							var bTerminalID = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								bTerminalID = inVals[i];
								outTxt += this.pindentComment("bTerminalID", 1);
								i++; j--;
							}
							var wTerminalType = -1;
							if (i < inVals.length && j > 0) {
							...
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("wTerminalType" + this.pTerminalType(wTerminalType), 2);
								i++; j--;
							}
							...
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("bAssocTerminal", 1);
								i++; j--;
							}
							var bNrChannels = -1;
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("bNrChannels " + bNrChannels.toString(10), 1);
								i++; j--;
							}
							...
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("wChannelConfig" + this.pChannelConfig(wChannelConfig), 2);
								i++; j--;
							}
							var iChannelNames = -1;
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("iChannelNames", 1);
								i++; j--;
							}
							var iTerminal = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								iTerminal = inVals[i];
								outTxt += this.pindentComment("iTerminal", 1);
								i++; j--;
							}
						}
						else if (bDescriptorSubtype == 0x03)
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_INTERFACE -> OUTPUT_TERMINAL)", 1);
							i++; j--;
							var bTerminalID = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
							...
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								bSourceID = inVals[i];
								outTxt += this.pindentComment("bSourceID", 1);
								i++; j--;
							}
							var iTerminal = -1;
							if (i < inVals.length && j > 0) {
						...
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_INTERFACE -> MIXER_UNIT)", 1);
							i++; j--;
							var bUnitID = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								bUnitID = inVals[i];
								outTxt += this.pindentComment("bUnitID", 1);
								i++; j--;
							}
							var bNrInPins = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								bNrInPins = inVals[i];
								outTxt += this.pindentComment("bNrInPins " + bNrInPins.toString(10), 1);
								i++; j--;
							}
							var pinIdx = 0;
							for (pinIdx = 0; pinIdx < bNrInPins; pinIdx++)
							{
								...
								if (i < inVals.length && j > 0) {
									outTxt += this.pHexC(inVals[i]);
									baSourceID = inVals[i];
									outTxt += this.pindentComment("baSourceID[" + pinIdx.toString(10) + "] = " + this.pHex(baSourceID), 1);
									i++; j--;
								}
							}
							var bNrChannels = -1;
							...
							while (j > 1)
							{
								...
								if (i < inVals.length && j > 0) {
									outTxt += this.pHexC(inVals[i]);
									bmControls = inVals[i];
									outTxt += this.pindentComment("bmControls", 1);
									i++; j--;
								}
							}
							var iMixer = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								iMixer = inVals[i];
								outTxt += this.pindentComment("iMixer", 1);
								i++; j--;
							}
						}
						else if (bDescriptorSubtype == 0x06)
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_INTERFACE -> FEATURE_UNIT)", 1);
							i++; j--;
							var bUnitID = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
							...
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("bControlSize " + bControlSize.toString(10), 1);
								i++; j--;
							}
							...
							for (bmaControlsIdx = 0; bmaControlsIdx < arrSz; bmaControlsIdx++)
							{
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("bmaControls[" + bmaControlsIdx.toString(10) + "]" + bmaAudCtrls, 2);
									i++; j--;
								}
							}
							var iFeature = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								iFeature = inVals[i];
								outTxt += this.pindentComment("iFeature", 1);
								i++; j--;
							}
						}
						else {
				...
				else if (bDescriptorType == 0x24 && bInterfaceClass == 0x01 && bInterfaceSubClass == 0x02)
				{
					...
					if (i < inVals.length && j > 0)
					{
						...
						if (bDescriptorSubtype == 0x01)
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_INTERFACE -> AS_GENERAL)", 1);
							i++; j--;
							var bTerminalLink = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								bTerminalLink = inVals[i];
								outTxt += this.pindentComment("bTerminalLink", 1);
								i++; j--;
							}
							var bDelay = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								bDelay = inVals[i];
								outTxt += this.pindentComment("bDelay " + bDelay.toString(10), 1);
								i++; j--;
							}
							var wFormatTag= -1;
							if (i < inVals.length && j > 0) {
							...
							if (i < inVals.length && j > 0) {
								...
								outTxt += this.pindentComment("wFormatTag" + this.pWaveFormat(wFormatTag), 2);
								i++; j--;
							}
						}
						else if (bDescriptorSubtype == 0x02)
						{
							outTxt += this.pindentComment("bDescriptorSubtype (CS_INTERFACE -> FORMAT_TYPE)", 1);
							i++; j--;
							var bFormatType = -1;
							if (i < inVals.length && j > 0) {
								outTxt += this.pHexC(inVals[i]);
								bFormatType = inVals[i];
								outTxt += this.pindentComment("bFormatType " + bFormatType.toString(10), 1);
								i++; j--;
							}
							if (bFormatType == 1 || bFormatType == 3)
							{
								...
								if (i < inVals.length && j > 0) {
									...
									bNrChannels = inVals[i];
									if (bNrChannels == 1) {
										outTxt += this.pindentComment("bNrChannels (Mono)", 1);
									}
									else if (bNrChannels == 2) {
										outTxt += this.pindentComment("bNrChannels (Stereo)", 1);
									}
									else {
										outTxt += this.pindentComment("bNrChannels " + bNrChannels.toString(10), 1);
									}
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("bSubFrameSize " + bSubFrameSize.toString(10), 1);
									i++; j--;
								}
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("bBitResolution " + bBitResolution.toString(10), 1);
									i++; j--;
								}
							}
							else if (bFormatType == 2)
							{
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("wMaxBitRate " + wMaxBitRate.toString(10) + " kbits/s", 2);
									i++; j--;
								}
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("wSamplesPerFrame " + wMaxBitRate.toString(10), 2);
									i++; j--;
								}
							}
							else
							...
							if (bSamFreqType == 0)
							{
								outTxt += this.pindentComment("bSamFreqType (Continuous)", 1);
								var tLowerSamFreq = -1;
								if (i < inVals.length && j > 0) {
									outTxt += this.pHexC(inVals[i]);
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("tLowerSamFreq " + tLowerSamFreq.toString(10) + " Hz", 2);
									i++; j--;
								}
								var tUpperSamFreq = -1;
								if (i < inVals.length && j > 0) {
								...
								if (i < inVals.length && j > 0) {
									...
									outTxt += this.pindentComment("tUpperSamFreq " + tUpperSamFreq.toString(10) + " Hz", 2);
									i++; j--;
								}
							}
							else
							{
								outTxt += this.pindentComment("bSamFreqType " + bSamFreqType.toString(10), 1);
								var freqCnter;
								for (freqCnter = 0; freqCnter < bSamFreqType && bSamFreqType > 0 && j > 0; freqCnter++)
								{
									...
									if (i < inVals.length && j > 0) {
										...
										outTxt += this.pindentComment("tSamFreq[" + (freqCnter + 1).toString(10) + "] " + tSamFreq.toString(10) + " Hz", 2);
										i++; j--;
									}
								}
							}
  ...
  go_parse_stdrequest(inTxt)
  {
	// TODO try
	{
		...
		if (inVals.length != 8) {
			this.possible_errors = 8; // all standard requests are supposed to be 8 bytes long
		}
		...
		for (var i = 0; i < inVals.length; )
		{
			if (i == 0)
			{
				...
				outTxt += this.pHexC(bmRequestType);
				...
				outTxt += this.pindentComment(str, 1);
			}
			else if (i == 1)
			{
				bRequest = inVals[i++];
				outTxt += this.pHexC(bRequest);
				var str = "bRequest";
				if (((bmRequestType >> 5) & 0x03) == 0x00)
				{
			...
			{
				wValue = inVals[i++];
				outTxt += this.pHexC(wValue);
				if (((bmRequestType >> 5) & 0x03) == 0x00 && (bRequest == 0x06 || bRequest == 0x07)) {
					outTxt += this.pindentComment("wValue[0:7]  Desc Index: " + wValue.toString(10), 1);
				}
			}
			else if (i == 3)
			{
				...
				if (((bmRequestType >> 5) & 0x03) == 0x00)
				{
					if (bRequest == 0x06 || bRequest == 0x07) {
						outTxt += this.pindentComment("wValue[8:15] Desc Type:" + this.pDescriptorType(inVals[i]), 1);
					}
					else if (bRequest == 0x01 || bRequest == 0x03) {
						outTxt += this.pindentComment("wValue Feature Selector: " + wValue.toString(10), 2);
					}
					else if (bRequest == 0x05) {
						outTxt += this.pindentComment("wValue Device Addr: " + wValue.toString(10), 2);
					}
					else if (bRequest == 0x09) {
						outTxt += this.pindentComment("wValue Config Num: " + wValue.toString(10), 2);
					}
					else if (bRequest == 0x0B) {
						outTxt += this.pindentComment("wValue Alt Setting: " + wValue.toString(10), 2);
					}
					else {
						outTxt += this.pindentComment("wValue = " + this.pHex(wValue), 2);
					}
				}
				else
				{
					outTxt += this.pindentComment("wValue[0:15] = " + this.pHex(wValue), 2);
				}
				i++;
			}
			else if (i == 4)
			...
			else if (i == 5)
			{
				...
				if (((bmRequestType >> 5) & 0x03) == 0x00)
				{
					if ((bmRequestType == 0x01 && bRequest == 0x01) || (bmRequestType == 0x81 && bRequest == 0x0A) || (bmRequestType == 0x81 && bRequest == 0x00) || (bmRequestType == 0x01 && bRequest == 0x03) || (bmRequestType == 0x01 && bRequest == 0x0B)) {
						outTxt += this.pindentComment("wIndex Interface: " + wIndex.toString(10), 2);
					}
						...
						outTxt += this.pindentComment("wIndex Endpoint: " + this.pHex(wIndex) + str, 2);
					}
					else if (bRequest == 0x06 || bRequest == 0x07) {
						outTxt += this.pindentComment("wIndex Language ID: " + this.pHex(wIndex), 2);
					}
					else {
						outTxt += this.pindentComment("wIndex = " + this.pHex(wIndex), 2);
					}
				}
				else
				{
			...
			{
				wLength = inVals[i++];
				outTxt += this.pHexC(wLength);
			}
			else if (i == 7)
			{
				wLength += inVals[i] << 8;
				outTxt += this.pHexC(inVals[i]);
				outTxt += this.pindentComment("wLength = " + wLength.toString(10), 2);
				i++;
			}
			else
			{
  ...
  pusagePage(s, v)
  {
  	if (s <= 0) {
  		...
  		if (v < tbl.length) {
  			...
  			this.possible_errors++;
  			return " (Reserved " + this.pHex(v) + ")";
  		}
  		else if (v >= 0xFF00 && v <= 0xFFFFF) {
  			return " (Vendor Defined " + this.pHex(v) + ")";
  		}
  		else {
  			return " (" + this.pHex(v) + ")";
  		}
  ...
  pUsage(s, v, u)
  {
  	if (s <= 0) {
          ...
          // TODO throw e;
  			...
  			// TODO try
        {
  				if (str === 0 || str == 0 || str == "" || str === undefined || str.trim() == "") {
  					this.possible_errors++;
  					str = this.pHex(v);
  				}
  			}
  ...
  sample_stdrequest()
  {
  	...
  	return this.go_parse_stdrequest(inStr);
  }

  best_guess(inTxt)
  {
  	var hid_errs, stddesc_errs, stdreq_errs;
  	this.go_parse_hidrepdesc(inTxt);
  	hid_errs = this.possible_errors;
  	this.go_parse_stddesc(inTxt);
  	stddesc_errs = this.possible_errors;
  	this.go_parse_stdrequest(inTxt);
  	stdreq_errs = this.possible_errors;
